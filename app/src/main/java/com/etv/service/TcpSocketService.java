...
//                MyLog.netty("=====心跳消息发送状态========" + isSuccess + " / " + errorDesc);
    ...
    public void backLineStaues(boolean isLineSuccess, String errorDesc) {
        ...
        if (!isLineSuccess) {
            sendBroadToUi(AppInfo.SOCKET_LINE_STATUS_CHANGE, SocketWebListener.SOCKET_ERROR, "Server Line Failed :" + errorDesc, null);
            return;
        }
        AppConfig.isOnline = true;
...
//        MapLocationParsener.getInstance(TcpSocketService.this).startLocationService(tag);
    ...
    private void clearTimerPowerOnOff() {
        if (powerOnOffUtil == null) {
            powerOnOffUtil = new PowerOnOffUtil(TcpSocketService.this);
        }
        DbTimerUtil.clearTimeDb();
